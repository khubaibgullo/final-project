...
const users = [
	...
	{
		name: "Sara Malik",
		email: "sara@learnhub.com",
		password: "pass1234",
		role: "instructor",
	},
	{
		name: "Usman Tariq",
		email: "usman@learnhub.com",
		password: "pass1234",
		role: "instructor",
	},
...
const buildCourses = (instructorIds) => {
	...
	return [
		...
		// ── Business ─────────────────────────────────────────────────────────────
		{
			...
			lessons: [
				{
					title: "Freelancing Mindset & Getting Started",
					duration: 20,
					order: 1,
				},
				{
					title: "Setting Up Fiverr & Upwork Profiles",
					duration: 25,
					order: 2,
				},
...
async function seed() {
	try {
		...
		console.log(
			`  📚 ${createdCourses.length} courses (${publishedCourses.length} published, ${createdCourses.length - publishedCourses.length} draft)`,
		);
		console.log(
			`  👤 ${createdUsers.length} users  |  📝 ${enrollments.length} enrollments`,
		);
		console.log("══════════════════════════════════════════\n");
