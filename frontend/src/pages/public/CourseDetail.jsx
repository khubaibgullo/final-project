...
const CourseDetail = () => {
  ...
  return (
    <div style={{ background: 'var(--bg-primary)', minHeight: '100vh' }}>
      {/* Hero */}
      <div style={{ background: 'var(--text-primary)', padding: '56px 24px 48px' }}>
        <div className="container">
          <div style={{ maxWidth: 680 }}>
            <span className="badge" style={{ background: 'rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.9)', marginBottom: 16 }}>
              {course.category}
            </span>
            <h1 style={{ fontSize: 'clamp(1.6rem,4vw,2.5rem)', color: 'white', marginBottom: 16, fontWeight: 800, letterSpacing: '-0.03em' }}>
              {course.title}
            </h1>
          ...
          <div>
            <div className="card" style={{ marginBottom: 24 }}>
              ...
              <div className="card-body">
                <p style={{ color: 'var(--text-secondary)', lineHeight: 1.75 }}>{course.description}</p>
              </div>
            </div>
            ...
            <div className="card">
              ...
              {(!course.lessons || course.lessons.length === 0) ? (
                <div className="card-body">
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>No lessons added yet.</p>
                </div>
              ) : (
                <div>
                  {course.lessons.map((lesson, i) => (
                    <div key={lesson._id} style={{
                      ...
                      padding: '14px 24px',
                      borderBottom: i < course.lessons.length - 1 ? '1px solid var(--border-light)' : 'none',
                    }}>
                      <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                        <span style={{
                          width: 26, height: 26, background: 'var(--accent-light)',
                          borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center',
                          fontSize: '0.72rem', color: 'var(--accent-text)', fontWeight: 700, flexShrink: 0,
                        }}>{i + 1}</span>
                        <span style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', fontWeight: 500 }}>{lesson.title}</span>
                      </div>
                      {lesson.duration > 0 && (
                        <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 500 }}>{lesson.duration} min</span>
                      )}
                    </div>
                  ))}
                </div>
            ...
            <div style={{ padding: '24px' }}>
              ...
              {message === 'success' && (
                <div className="alert alert-success" style={{ fontSize: '0.8rem' }}>
                  Enrolled! Go to your <a href="/student/dashboard" style={{ color: 'inherit', fontWeight: 700 }}>dashboard</a> to start.
                </div>
              )}
